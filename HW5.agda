...
collaborated on high level ideas about E1 and E2 with Tyler Ehrlich
-}
...
homom[reverse/⧺] (x ∷ xs) ys rewrite homom[reverse/⧺] xs ys | assoc[⧺] (reverse ys) (reverse xs) [ x ] = ↯
...
invol[reverse] [] = ↯
invol[reverse] (x ∷ xs) rewrite homom[reverse/⧺] (reverse xs) [ x ] | invol[reverse] xs = ↯
...
unit[map] [] = ↯
unit[map] (x ∷ xs) rewrite unit[map] xs = ↯
-- E4: [★★☆]
...
homom[map/∘] g f [] = ↯
homom[map/∘] g f (x ∷ xs) rewrite homom[map/∘] g f xs = ↯
...
homom[map/⧺] f [] ys = ↯
homom[map/⧺] f (x ∷ xs) ys rewrite homom[map/⧺] f xs ys = ↯
...
commu[map/reverse] f [] = ↯
commu[map/reverse] f (x ∷ xs) rewrite homom[map/⧺] f (reverse xs) [ x ] | commu[map/reverse] f xs = ↯
...
homom[sum/⧺] [] ys = ↯
homom[sum/⧺] (x ∷ xs) ys rewrite homom[sum/⧺] xs ys | assoc[+] x (sum xs) (sum ys) = ↯

-- EXTRA PROBLEMS (not graded)
--
-- 1. prove that (sum (reverse xs)) ≡ sum xs
e1 : ∀ (xs : list ℕ) → sum( (reverse xs)) ≡ sum xs
e1 [] = ↯
e1 (x ∷ xs) with e1 xs
… | IH rewrite homom[sum/⧺] (reverse xs) [ x ] | IH | runit[+] x | commu[+] (sum xs) x = ↯
  --runit[+] : ∀ (m : ℕ) → m + 0 ≡ m
  --assoc[+] : ∀ (m n p : ℕ) → m + (n + p) ≡ (m + n) + p
  --commu[+] : ∀ (m n : ℕ) → m + n ≡ n + m
-- 2. define:
-- 
singleton : ∀ {A : Set} → list A
singleton = []

concat-map : ∀ {A B : Set} → list A → (A → list B) → list B
concat-map [] f = []
concat-map (x ∷ a) f = {!!}
