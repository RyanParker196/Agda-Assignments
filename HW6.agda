{-
Name: Ryan Parker ﹙⊙︿⊙﹚_/¯ and Tyler Ehrlich
Date: 10/6/19

Collaboration & Resources Statement:
Partner: Tyler Ehrlich and I worked on this entire homework together
-}
...
find-min-lower-bound-first x [] = xRx[≤] x
find-min-lower-bound-first x (y ∷ ys) with x ≤⋆ y
find-min-lower-bound-first x (y ∷ ys) | [≤] x<y rewrite cmp[≤?/≤] x<y = find-min-lower-bound-first x ys
find-min-lower-bound-first x (y ∷ ys) | [>] y<x rewrite cmp[≤?/>] y<x with find-min-lower-bound-first y ys | wk[<] y<x | _⊚[≤]_
… | IH | H1 | H2 = H2 H1 IH 
...
find-min-lower-bound x [] = []
find-min-lower-bound x (y ∷ ys) with x ≤⋆ y
find-min-lower-bound x (y ∷ ys) | [≤] x<=y rewrite cmp[≤?/≤] x<=y with find-min-lower-bound x ys | find-min-lower-bound-first x ys | _⊚[≤]_
… | IH | H1 | H2 = H2 x<=y H1 ∷ IH
find-min-lower-bound x (y ∷ ys) | [>] y<x rewrite cmp[≤?/>] y<x with find-min-lower-bound y ys | find-min-lower-bound-first y ys | _⊚[≤]_ | wk[<]
… | IH | H1 | H2 | H3 = H2 (H3 y<x) H1 ∷ IH
...
find-min-upper-bound w x [] εˣ εsʸ = εˣ
find-min-upper-bound w x (y ∷ ys) εˣ εsʸ with x ≤⋆ y
find-min-upper-bound w x (y ∷ ys) εˣ (ey ∷ εsʸ) | [≤] x<=y rewrite cmp[≤?/≤] x<=y = find-min-upper-bound w x ys εˣ εsʸ
find-min-upper-bound w x (y ∷ ys) εˣ (ey ∷ εsʸ) | [>] y>x rewrite cmp[≤?/>] y>x = find-min-upper-bound w y ys ey εsʸ 
...
ssort-invariant x [] [] = []
ssort-invariant x (y ∷ ys) (ε ∷ εs) = ⟨ find-min-upper-bound x y ys ε εs ⟩
...
sorted[ssort] [] = []
sorted[ssort] (x ∷ xs) with find-min-lower-bound x xs | ssort-invariant (π₁ (find-min x xs)) (π₂ (find-min x xs)) | sorted[ssort] xs
… | E2 | E4 | IH = E4 E2  ∷ sorted[ssort] (π₂ (find-min x xs))
