...
perm[merge] [] (ys) rewrite lunit[⊎] (bag-list ys) = ↯
perm[merge] (xs) [] rewrite runit[merge] xs | runit[⊎] (bag-list xs) = ↯
perm[merge] (x ∷ xs) (y ∷ ys) with x ≤? y
perm[merge] (x ∷ xs) (y ∷ ys) | [≤] rewrite perm[merge] xs (y ∷ ys) = assoc[⊎] (❴ x ❵) (bag-list xs) (❴ y ❵ ⊎ bag-list ys)
perm[merge] (x ∷ xs) (y ∷ ys) | [>] rewrite perm[merge] (x ∷ xs) ys
  | assoc[⊎] (❴ y ❵) (❴ x ❵ ⊎ bag-list xs) (bag-list ys)
  | assoc[⊎] (❴ x ❵ ⊎ bag-list xs) (❴ y ❵) (bag-list ys)
  | commu[⊎] (❴ y ❵) (❴ x ❵ ⊎ bag-list xs)= ↯
...
perm[msort-rec] [] a = ↯
perm[msort-rec] [ x ] (Acc a) = ↯
perm[msort-rec] (x1 ∷ x2 ∷ xs) (Acc a) with split xs | split-length xs | perm[split] xs
perm[msort-rec] (x1 ∷ x2 ∷ xs) (Acc a) | ⟨ z1 , z2 ⟩ | ⟨ v1 , v2 ⟩ | H3 rewrite perm[merge]
  (msort-rec (x1 ∷ z1) (a (S (s[<] (length xs) ⊚[</≤] v1))))
  (msort-rec (x2 ∷ z2) (a (S (s[<] (length xs) ⊚[</≤] v2)))) with perm[msort-rec] (x1 ∷ z1) (a (S (s[<] (length xs) ⊚[</≤] v1)))
… | H rewrite H with perm[msort-rec] (x2 ∷ z2) (a (S (s[<] (length xs) ⊚[</≤] v2)))
… | W rewrite W | H3
  | assoc[⊎] (❴ x1 ❵ ⊎ bag-list z1) (❴ x2 ❵) (bag-list z2) | assoc[⊎] (❴ x2 ❵) (bag-list z1) (bag-list z2)
  | commu[⊎] (❴ x2 ❵) (bag-list z1)
  | assoc[⊎] (❴ x1 ❵) (bag-list z1 ⊎ ❴ x2 ❵) (bag-list z2)
  | assoc[⊎] (❴ x1 ❵) (bag-list z1) (❴ x2 ❵) = ↯


-- E4: [★☆☆]
-- Prove that merge-sort returns a permutation of the input list.
-- HINTS:
-- ‣ do not do case analysis or induction
-- ‣ use E3 and wf[<]
perm[msort] : ∀ (xs : list ℕ) → bag-list (msort xs) ≡ bag-list xs
perm[msort] xs = perm[msort-rec] xs (wf[<] (length xs))
