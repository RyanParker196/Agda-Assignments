...
split-length [] = ⟨ Z , Z ⟩
split-length (x ∷ xs) with split-length xs
split-length (x ∷ xs) | ⟨ p1 , p2 ⟩ with _⊚[≤]_ | s[≤] (length xs)
… | H | H2 = ⟨ H H2 p2 , S p1 ⟩
...
merge-bounded x [] [] ε₁ ε₂ = []
merge-bounded x [] (z ∷ zs) ε₁ ⟨ ε2 ⟩ = ⟨ ε2 ⟩
merge-bounded x (y ∷ ys) [] ⟨ ε1 ⟩ ε₂ = ⟨ ε1 ⟩
merge-bounded x (y ∷ ys) (z ∷ zs) ⟨ ε1 ⟩ ⟨ ε2 ⟩ with y ≤? z
merge-bounded x (y ∷ ys) (z ∷ zs) ⟨ ε1 ⟩ ⟨ ε2 ⟩ | [≤] = ⟨ ε1 ⟩
merge-bounded x (y ∷ ys) (z ∷ zs) ⟨ ε1 ⟩ ⟨ ε2 ⟩ | [>] = ⟨ ε2 ⟩
-- E3: [★★★]
...
merge-sorted [] [] [] [] = []
merge-sorted [] (x ∷ ys) [] (ε1 ∷ εs₂) = ε1 ∷ εs₂
merge-sorted (x ∷ xs) [] (ε1 ∷ εs₁) [] = ε1 ∷ εs₁
merge-sorted (x ∷ xs) (y ∷ ys) (ε1 ∷ εs₁) (ε2 ∷ εs₂) with x ≤⋆ y
merge-sorted (x ∷ xs) (y ∷ ys) (ε1 ∷ εs₁) (ε2 ∷ εs₂) | [≤] z
  rewrite cmp[≤?/≤] z = merge-bounded x xs (y ∷ ys) ε1 ⟨ z ⟩ ∷ merge-sorted xs (y ∷ ys) εs₁ (ε2 ∷ εs₂)
merge-sorted (x ∷ xs) (y ∷ ys) (ε1 ∷ εs₁) (ε2 ∷ εs₂) | [>] z
  rewrite cmp[≤?/>] z = merge-bounded y (x ∷ xs) ys ⟨ wk[<] z ⟩ ε2 ∷ merge-sorted (x ∷ xs) ys (ε1 ∷ εs₁) εs₂
...
msort-sorted-rec [] a = []
msort-sorted-rec [ x ] a = [] ∷ []
msort-sorted-rec (x ∷ x₁ ∷ xs) (Acc a) with split xs | split-length xs
… | ⟨ ys , zs ⟩ | ⟨ H1 , H2 ⟩ =
  let p1 = (a (S (s[<] (length xs) ⊚[</≤] H1)))
  in merge-sorted _ _ (msort-sorted-rec (x ∷ ys) p1) (msort-sorted-rec (x₁ ∷ zs) _)
...
msort-sorted xs with msort-sorted-rec xs | wf[<] (length xs)
… | E4 | W = E4 W

--msort-sorted-rec xs (Acc λ x → {!wf[<] (.y)!})
